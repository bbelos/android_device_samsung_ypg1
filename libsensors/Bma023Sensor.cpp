...
int Bma023Sensor::readEvents(sensors_event_t* data, int count)
{
    //LOGD("Bma023Sensor::~readEvents() %d", count);
    if (count < 1)
        return -EINVAL;
	...
	//LOGD("Bma023Sensor::~readEvents() numEventReceived = %d", numEventReceived);
